...
import { useCart, PreOrderOccasion } from '@/contexts/CartContext';
import { Button } from './ui/Button';

interface PreOrderSetupModalProps {
    isOpen: boolean;
    onClose: () => void;
    onComplete: () => void;
}

export const PreOrderSetupModal: React.FC<PreOrderSetupModalProps> = ({ isOpen, onClose, onComplete }) => {
    const { branches, selectedBranch, setBranch } = useBranch();
    const { setPreOrderDate, setPreOrderOccasion, setOrderType } = useCart();

    const [date, setDate] = useState('');
    const [occasion, setOccasion] = useState<PreOrderOccasion>('PERSONAL');
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    ...
                                    <button
                                        key={type.id}
                                        onClick={() => setOccasion(type.id as PreOrderOccasion)}
