...
export default function HomePage() {
  ...
  const handleModeSelection = (mode: 'IMMEDIATE' | 'PRE_ORDER') => {
    if (mode === 'IMMEDIATE') {
  ...
  const handleOrderTypeSelect = (type: 'IMMEDIATE' | 'PRE_ORDER') => {
    ...
    if (type === 'PRE_ORDER') {
      ...
      if (pendingProduct) {
        ...
        // Needs careful recursion handling or just call addToCart directly?
  ...
  const getSection = (category: string) => products.filter(p => p.specialCategory === category);
  const getOthers = () => products.filter(p => !p.specialCategory);
...
const ProductSection = ({ title, subtitle, products, onAdd, isGrid = false, buttonLabel }: any) => {
